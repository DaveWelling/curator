...
export function publish(action, preAction = false) {
...
export const eventSink = () => next => (action) => {
    publish(action);
    return next(action);
};

export const preActionEventSink = () => next => (action) => {
    publish(action, true);
    return next(action);
};

export function unsubscribe(action, listener, preAction) {
    let index = action;
    if (preAction) {
        index = 'PRE_' + action;
    }
    const subscription = sink[index] || [];
...
export function subscribe(action, listener, preAction = false) {
    let index = action;
    if (preAction) {
        index = 'PRE_' + action;
    }
